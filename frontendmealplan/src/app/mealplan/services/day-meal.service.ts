...
export class DayMealService {
  ...
  public getDayPlanMeals(dayNumber: number, userid: number) {
    return this.apiService.get<DayMeal[]>(
  ...
  public getTotalDayNutrition(dayNumber: number, userid: number) {
    return this.apiService.get<string[]>(
      this.DAYNUTRITION_URL + `${dayNumber}`
    );
  }
  ...
  public saveFeedback(
    ...
    return this.apiService.post<UserFeedback>(
      this.SAVEFEEDBACK_URL + `${mealId}`,
      userFeedback
    );
  }

  public getIngredients(mealId: number) {
    return this.apiService.get<MealIngredients[]>(
      this.INGREDIENTS_URL + `${mealId}`
    );
  }
